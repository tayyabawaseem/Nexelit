...
HTML;
    ...
    public function cartSummery() : string
    {
        ...
        if ($this->count() > 0) {
            ...
            $coupon_remove_markup = $this->coupon() > 0 ? '<span class="text-danger ml-2 cursor-pointer" role="button" id="remove_coupon_from_cart">remove</span>' : '';

            $output .= ' <tr><th>' . __('Coupon Discount') . '</th><td>-' . amount_with_currency_symbol($this->coupon()) . $coupon_remove_markup.'</td></tr> ';
