...
ITEM;
    ...
    public function tags_wise_blog_page($tag)
    {
        ...
        $all_blogs = Blog::where('lang', $lang)
            ...
            ->whereRaw("FIND_IN_SET(?, REPLACE(tags, ' ', '-'))", [str_replace(' ', '-', $tag)])
            ->orderBy('id', 'desc')
    ...
    public function service_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_services = Services::where('status', 'publish')
            ->where('lang', $lang)
            ->orderBy('sr_order', 'asc')
            ->paginate(get_static_option('service_page_service_items'));
    ...
    public function work_page()
    {
        ...
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_work = Works::where(['status'=> 'publish', 'lang' => $lang])->orderBy('id', 'desc')->paginate(12);
    ...
    public function products(Request $request)
    {
        ...
        $maximum_available_price = max(Products::orderBy('sale_price','desc')->pluck('sale_price')->toArray());
    ...
    public function knowledgebase()
    {
        ...
        $all_knowledgebase_category = KnowledgebaseTopic::where('status', 'publish')
            ->where(['lang' => $lang])
            ->limit(get_static_option('site_knoeledgebase_post_items'))
            ->get();

        $ids_knowledgebase_category = $all_knowledgebase_category->pluck('id')->toArray();

        $all_knowledgebase = Knowledgebase::where('status','publish')
            ->where(['lang' => $lang])
            ->whereIn('topic_id', $ids_knowledgebase_category)
            ->paginate(20)
            ->groupby('topic_id');

        $popular_articles = Knowledgebase::where('status', 'publish')
            ->where(['lang' => $lang])
            ->orderBy('views', 'desc')
            ->get()
            ->take(5);
    ...
    public function product_download(Request $request, $id){
        ...
        if (!is_null($product_details)){
            $assets_path = dirname(base_path()) . '/assets';
            $source_file = $assets_path . '/uploads/downloadable/' . $product_details->downloadable_file;

            if (File::exists($source_file)){
                $file_extensions = File::extension($source_file);
                $new_filename = \Str::slug($product_details->title) . '.' . $file_extensions;

                // Stream the file directly without copying it
                $response = response()->stream(function() use ($source_file) {
                    ...
                    'Content-Type' => File::mimeType($source_file),
                    'Content-Disposition' => 'attachment; filename="' . $new_filename . '"',
                    'Content-Length' => File::size($source_file),
                    'Accept-Ranges' => 'bytes',
                ]);

                return $response;
            } else {
                return back()->with(['msg' => __('File not found'), 'type' => 'error']);
            }
        }
        return back()->with(['msg' => __('Product not found'), 'type' => 'error']);
    }
