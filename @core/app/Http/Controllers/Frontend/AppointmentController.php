...
class AppointmentController extends Controller
{
    ...
    public function appointment_booking_time_check(Request $request){
        ...
        $avialable_times = $appointment_details->booking_time_ids;
        $selected_avialable_times = $appointment_details->booking_time_ids;

        $all_booking = AppointmentBooking::where([
            "appointment_id" => $request->appointment_id,
            "booking_date" => $request->date,
        ])->get();
        $all_booking_time = AppointmentBookingTime::whereIn(
            "id",
            array_column($selected_avialable_times, "id")
        )->get();

        if ($all_booking->count() >= 1) {
            // has some booking int this date
            //check for maximum appointment
            $allboking_time_array = $all_booking
                ->pluck("booking_time_id")
                ->toArray();
            $all_booking_time = $all_booking_time->map(function ($item) use (
                $allboking_time_array
            ) {
                return !in_array($item->id, $allboking_time_array)
                    ? $item
                    : false;
            });
        }
