...
class AppointmentBookingController extends Controller
{
    ...
    public function booking(Request $request){
        ...
        if(empty(get_static_option('site_payment_gateway')) || $appointment->price <= 0 ){
            //todo add filter if client disabled all payment gateway or if the price is 0 or less than zero
        ...
        elseif ($request->selected_payment_gateway === 'senangpay') {
            $params = $this->common_charge_customer_data($new_appointment,route('frontend.appointment.senangpay.ipn'));
        ...
        elseif ($request->selected_payment_gateway === 'iyzipay') {
            $params = $this->common_charge_customer_data($new_appointment,route('frontend.appointment.iyzipay.ipn'));
        ...
        elseif ($request->selected_payment_gateway === 'awdpay') {
            $params = $this->common_charge_customer_data($new_appointment,route('frontend.appointment.awdpay.ipn'));
