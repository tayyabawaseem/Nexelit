...
class GeneralSettingsController extends Controller
{
    ...
    {
        $request->validate([
            'custom_font_name' => 'required|string|max:255',
            'custom_font_variants' => 'nullable|string',
        ]);

        $customFontName = trim($request->custom_font_name);

        // Load Google Fonts JSON
        $google_fonts_json = file_get_contents('assets/frontend/webfonts/google-fonts.json');
        $google_fonts = json_decode($google_fonts_json, true);

        // Check for duplicates in Google Fonts
        if (array_key_exists($customFontName, $google_fonts)) {
            return back()->with(['msg' => __('This font already exists in Google Fonts.'), 'type' => 'danger']);
        }

        // Check for duplicates in DB
        if (CustomFont::where('name', $customFontName)->exists()) {
            return back()->with(['msg' => __('This font already exists.'), 'type' => 'danger']);
        }

        // Save in DB
        CustomFont::create([
            'name' => $customFontName,
            'variants' => $request->custom_font_variants ? explode(';', $request->custom_font_variants) : ['400']
        ]);

        return back()->with(['msg' => __('Font added successfully'), 'type' => 'success']);
    }
