...
class PaymentLogController extends Controller
{
    ...
    public function order_payment_form(Request $request)
    {
        ...
        elseif ($request->payment_gateway === 'senangpay') {
            $params = $this->common_charge_customer_data($payment_details,$request,route('frontend.price.plan.senangpay.ipn'));
            $senangpay = PaymentGatewayCredential::get_senangpay_credential();
            $response = $senangpay->charge_customer($params);

            return $response;
        }

        elseif ($request->payment_gateway === 'iyzipay') {
            $params = $this->common_charge_customer_data($payment_details,$request,route('frontend.price.plan.iyzipay.ipn'));
            $iyzipay = PaymentGatewayCredential::get_iyzipay_credential();
            $response = $iyzipay->charge_customer($params);

            return $response;
        }

        elseif ($request->selected_payment_gateway === 'awdpay') {
            $params = $this->common_charge_customer_data($payment_details,$request,route('frontend.price.plan.awdpay.ipn'));
            $awdpay = PaymentGatewayCredential::get_awdpay_credential();
            $response = $awdpay->charge_customer($params);
    ...
    {
        $successId = ($request->payment_gateway === 'billplz' || $request->payment_gateway === 'payfast')
        ...
        $data = [
            ...
            'success_url' => route(self::SUCCESS_ROUTE, random_int(333333, 999999) . $successId . random_int(333333, 999999)),
