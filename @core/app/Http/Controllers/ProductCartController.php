...
class ProductCartController extends Controller
{
    ...
    {
        // If "Display Price Only For Logged In User" is enabled, guests cannot add to cart
        if (!empty(get_static_option('display_price_only_for_logged_user')) && !auth()->check()) {
            return redirect()->back()->with([
                'msg'  => __('You must be logged in to add products to cart.'),
                'type' => 'danger' // or 'error' depending on your flash message styling
            ]);
        }
    ...
    public function ajax_add_to_cart(Request $request)
    {
        ...
        if (!empty(get_static_option('display_price_only_for_logged_user')) && !auth()->check()) {
            return response()->json([
                'status'  => 'error',
                'message' => __('You must be logged in to add products to cart.')
            ], 403);
        }
    ...
    public function product_coupon_remove(Request $request){
        
        session()->forget('coupon_discount');
        return  response()->json([
            'cart_total_markup' => Cart::cartSummery(),
            'status' => 'ok',
            'msg' => __('Coupon Removed'),
        ]);
    }
}
