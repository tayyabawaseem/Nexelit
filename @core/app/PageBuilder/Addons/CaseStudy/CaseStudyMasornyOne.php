...
class CaseStudyMasornyOne extends PageBuilderBase
{
    ...
    public function frontend_render()
    {
        ...
        $all_contain_cat = isset($all_case_study_by_category) ? $all_case_study_by_category->map(function ($item) { return $item->categories_id;})->toArray() : '';
        if (!empty($categories)){
...
HTML;
        ...
        if($filter_menu_status){
            ...
            foreach($all_work_category as $cat){
                $filter_menu_markup .= '<li data-filter=".'.Str::slug($cat->name,'-',null).'" >'.$cat->name.'</li>';
            }
