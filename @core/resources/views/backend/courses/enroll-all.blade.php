...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="#"
                                               ...
                                               data-toggle="modal"
                                               data-target="#enroll_status_change_modal"
                                               class="btn btn-xs btn-info btn-sm mb-3 mr-1 enroll_status_change_btn"
                                            >
                                                {{__("Update Status")}}
                                            </a>
    ...
    <div class="modal fade" id="enroll_status_change_modal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{__('Enroll Status Change')}}</h5>
                    <button type="button" class="close" data-dismiss="modal"><span>×</span></button>
                </div>
                <x-error-msg/>
                <form action="{{route('admin.course.enroll.status.change')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="enroll_id" id="enroll_id">
                        <div class="form-group">
                            <label for="enroll_status">{{__('Enroll Status')}}</label>
                            <select name="enroll_status" class="form-control" id="enroll_status">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            $(document).on('click','.enroll_status_change_btn',function(e){
                e.preventDefault();
                var el = $(this);
                var form = $('#enroll_status_change_modal');
                form.find('#enroll_id').val(el.data('id'));
                form.find('#enroll_status option[value="'+el.data('status')+'"]').attr('selected',true);
            });
        });
    </script>
@endsection
